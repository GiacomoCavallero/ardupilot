...
void AP_WindVane_Backend::speed_update_frontend(float apparent_speed_in)
{
    ...
    if (is_positive(_frontend._speed_filt_hz)) {
        _frontend._speed_apparent_filt.set_cutoff_frequency(_frontend._speed_filt_hz);
        _frontend._speed_apparent = _frontend._speed_apparent_filt.apply(apparent_speed_in, 0.02f);
    } else {
...
void AP_WindVane_Backend::speed_true_update_frontend(float true_speed_in)
{
    ...
    if (is_positive(_frontend._speed_filt_hz)) {
        _frontend._speed_true_filt.set_cutoff_frequency(_frontend._speed_filt_hz);
        _frontend._speed_true = _frontend._speed_true_filt.apply(true_speed_in, 0.02f);
    } else {
...
void AP_WindVane_Backend::direction_true_update_frontend(float true_angle_ef)
{
    ...
    if (is_positive(_frontend._dir_filt_hz)) {
        _frontend._direction_true_sin_filt.set_cutoff_frequency(_frontend._dir_filt_hz);
        _frontend._direction_true_cos_filt.set_cutoff_frequency(_frontend._dir_filt_hz);
        // https://en.wikipedia.org/wiki/Mean_of_circular_quantities
        const float filtered_sin = _frontend._direction_true_sin_filt.apply(sinf(true_angle_ef), 0.05f);
        const float filtered_cos = _frontend._direction_true_cos_filt.apply(cosf(true_angle_ef), 0.05f);
    ...
    const float tack_direction_apparent = wrap_PI(true_angle_ef - AP::ahrs().yaw);
