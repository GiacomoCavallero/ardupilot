...
#define GPS_ID (pmv->src == gps_primary_id ? "Primary" : (pmv->src == gps_secondary_id ? "Secondary" : (pmv->src == gps_tertiary_id ? "Tertiary" : "Unknown")))
...
{
    // Construct a timestamped var for adding to the buffer
    var_ = var;
...
T FiltBoxcar<T>::addPoint(T point)
{
    ...
    // Trim off old points
    while (!sample.empty() && (point.timestamp() - sample.back().timestamp() > bctime))
    {
        sample.pop_back();
    }
...
T FiltExp<T>::addPoint(T point)
{
    ...
    double dTime = std::chrono::duration_cast<std::chrono::seconds>(newPoint.timestamp()-oldPoint.timestamp());
...
T FiltExpNl<T>::addPoint(T point)
{
    ...
    if (bFac > 1)
    {
        // Reduce damping to 0 by the time we are 8 times bounds - for faster response during manouvers 
